...
- 筑基 AI 提供模型套餐、灵石额度、调用日志、文档和客户端导入入口。
- 新的 API 接入优先走筑基AI 分站 / Sub 道场：[https://sub.iaigc.fun](https://sub.iaigc.fun)。
- Codex / CC Switch / Cherry Studio 等 OpenAI 兼容客户端使用：`https://sub.iaigc.fun/v1`。
- API Key 从 `https://sub.iaigc.fun/keys` 创建或复制；不要把主站旧 token 和 Sub endpoint 混用。
- 用户应该先在 Sub 道场注册登录，再创建 API Key，然后把 Codex Provider 配到筑基。
...
3. 读取输出里的 `zhuji_provider_detected`：
   ...
   - `false`：停止筑基专属动作，引导注册、创建 Sub 道场 API Key、配置 Provider。
4. 根据用户目标分流到对应 skill。
...
| 安装本地插件、Codex 插件灰屏、官方 Apps/Plugins 不可用 | `zhuji-codex-plugins` |
...
- **开启 Codex 插件**：安装/启用本地插件；官方 Apps 需要时再处理登录态和模型 Provider 分离。
...
- 用户知道主站、Sub 道场 API Base 和 API Key 位置。
