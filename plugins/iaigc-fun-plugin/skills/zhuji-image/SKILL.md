...
- 如果当前环境没有可用 Sub 道场 API Key，只给出可运行命令和需要设置的环境变量。
...
兼容 OpenAI SDK 的环境变量名可以作为兜底，但值仍应是 Sub 道场 API Key，不是官方 OpenAI Key：

```bash
OPENAI_API_KEY="<ZHUJI_API_KEY>" ZHUJI_BASE_URL="https://sub.iaigc.fun/v1" \
